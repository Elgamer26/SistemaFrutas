...
while ($dat = $datos->fetch_object()) {
    try {
        ...
        $mail->SMTPAuth   = true;                                  
        $mail->Username = 'ise@instechsystema.i-sistener.xyz'; 
        $mail->Password = 'abc123ABC!';                           
        $mail->SMTPSecure = 'ssl';        
        $mail->Port       = 465;                    

        //Recipients
        $mail->setFrom('ise@instechsystema.i-sistener.xyz', 'InseTech');
        $mail->addAddress($dat->correo, 'InseTech');     // Add a recipient
